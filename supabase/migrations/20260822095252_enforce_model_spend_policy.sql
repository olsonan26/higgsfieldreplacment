-- Every model must have an admin-configured conservative reservation
-- amount before it can spend. The client cannot self-report an estimate.

create table public.workspace_model_spend_policies (
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  model_capability_id uuid not null references public.model_capabilities(id) on delete restrict,
  estimated_credit_reserve numeric(14,4) not null check (estimated_credit_reserve > 0),
  enabled boolean not null default false,
  created_by uuid not null references public.profiles(id) on delete restrict,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  primary key (workspace_id, model_capability_id)
);
create index workspace_model_spend_policies_capability_idx on public.workspace_model_spend_policies(model_capability_id);
create index workspace_model_spend_policies_created_by_idx on public.workspace_model_spend_policies(created_by);
create trigger workspace_model_spend_policies_set_updated_at
  before update on public.workspace_model_spend_policies
  for each row execute function private.set_updated_at();

alter table public.workspace_model_spend_policies enable row level security;
create policy workspace_model_spend_policies_select
  on public.workspace_model_spend_policies for select to authenticated
  using ((select private.is_workspace_member(workspace_id)));
create policy workspace_model_spend_policies_insert
  on public.workspace_model_spend_policies for insert to authenticated
  with check (created_by = (select auth.uid())
    and (select private.has_workspace_role(workspace_id, array['owner','admin']::public.workspace_role[])));
create policy workspace_model_spend_policies_update
  on public.workspace_model_spend_policies for update to authenticated
...
create policy workspace_model_spend_policies_delete
  on public.workspace_model_spend_policies for delete to authenticated
  using ((select private.has_workspace_role(workspace_id, array['owner','admin']::public.workspace_role[])));

revoke all on public.workspace_model_spend_policies from anon, authenticated;
grant select, insert, update, delete on public.workspace_model_spend_policies to authenticated;
