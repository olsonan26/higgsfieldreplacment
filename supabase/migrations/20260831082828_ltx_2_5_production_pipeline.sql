-- Supersede the initial fast-only fixture with a versioned production-quality
-- DFR profile. Capability history remains present for immutable generation
-- snapshots. This narrowly suspends only the capability immutability trigger
-- so the previously enabled version can be retired.
alter table public.model_capabilities disable trigger capability_immutable;
update public.model_capabilities
set enabled = false
where app_model_key = 'ltx-2-5' and version = 1 and enabled;
