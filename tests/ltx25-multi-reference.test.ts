...
function imageReference(index: number, role: "first_frame" | "reference_image" = "reference_image"): ReferenceInput {
  return {
...
describe("LTX-2.5 prompt image anchors", () => {
  it("accepts nine ordered prompt images and maps them to deterministic keyframes", () => {
    ...
    expect(images.slice(1).every((image) => image.frame_index % 8 === 0)).toBe(true);
